  ...
  return useMutation(productClient.create, {
    onSuccess: async () => {
      const generateRedirectUrl = router.query.shop
        ? `/${router.query.shop}${Routes.myproduct.list}`
        : Routes.myproduct.list;
...
export const useMyProductsQuery = (
  ...
  const { data, error, isLoading } = useQuery<ProductPaginator, Error>(
    [API_ENDPOINTS.MY_PRODUCTS, params],
    ({ queryKey, pageParam }) => productClient.myProducts(Object.assign({}, queryKey[1], pageParam)),
    {
      keepPreviousData: true,
      ...options
    }
