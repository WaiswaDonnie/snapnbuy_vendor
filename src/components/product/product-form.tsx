...
import {
  ...
  useGenerateProductNameMutation,
...
export default function CreateOrUpdateProductForm({
  ...
  const {
    ...
    isLoading,
    data, } = useGenerateProductNameMutation()
  ...
  const handleGenerateProductName = useCallback((imageUrl: string) => {
    if (!isEmpty(imageUrl)) {
      console.log('Generating product name', imageUrl);
      createAutoGeneratedPrompt({ imageUrl });
    }
  }, [productName]);
              ...
              <div className="relative mb-5">
                ...
                {isLoading ?
                  ...
                  : <OpenAIButton
                    ...
                    title={t('form:button-label-name-ai')}
                    onClick={() => {
                      let featuredImage = watch('image')?.original; // Get the value of the featured image field
                      if (featuredImage) {
                        handleGenerateProductName(featuredImage)
                      } else {
                        toast.error('Please upload an image to proceed with generating the product name')
                      }
                      console.log('Featured Image:', featuredImage);
                    }}
                  />}
                {/* <small style={{fontStyle:"italic"}} className='text-[12px] italise'>Make sure you upload an image to proceed with generating the product name.</small> */}
